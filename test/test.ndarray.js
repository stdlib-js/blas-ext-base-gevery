...
tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( gevery.length, 4, 'has expected arity' );
	t.end();
});
...
tape( 'the function tests whether every element in a strided array is truthy (accessors)', function test( t ) {
	...
	x = [ 1, 1, 1, 1 ];
	out = gevery( x.length, toAccessorArray( x ), 1, 0 );
	t.strictEqual( out, true, 'returns expected value' );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `false` (accessors)', function test( t ) {
	...
	out = gevery( 0, toAccessorArray( x ), 1, 0 );
	t.strictEqual( out, false, 'returns expected value' );

	out = gevery( -4, toAccessorArray( x ), 1, 0 );
	t.strictEqual( out, false, 'returns expected value' );
...
tape( 'the function supports a `stride` parameter (accessors)', function test( t ) {
	...
	out = gevery( 4, toAccessorArray( x ), 2, 0 );
	t.strictEqual( out, false, 'returns expected value' );
...
tape( 'the function supports an `offset` parameter (accessors)', function test( t ) {
	...
	out = gevery( 4, toAccessorArray( x ), 1, 1 );
	t.strictEqual( out, true, 'returns expected value' );
...
tape( 'the function supports a stride of `0` (accessors)', function test( t ) {
	...
	x = [ 1, 999, 999, 999 ];
	out = gevery( 4, toAccessorArray( x ), 0, 0 );
	t.strictEqual( out, true, 'returns expected value' );
...
tape( 'the function supports negative strides (accessors)', function test( t ) {
	...
	out = gevery( 4, toAccessorArray( x ), -1, 3 );
	t.strictEqual( out, false, 'returns expected value' );
